...
exports.default = (container) => {
    ...
    schema_1.UserTC.addResolver({
        ...
        resolve: ({ obj, args, context, info }) => __awaiter(this, void 0, void 0, function* () {
            ...
            let permissionsNames = yield permissionRepo.findByIds(user.data.permissions, 1000);
            if (permissionsNames.length > 0) {
                permissionsNames = permissionsNames.map((permission) => {
    ...
    schema_1.UserTC.addResolver({
        ...
        resolve: ({ obj, args, context, info }) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (permissions.length > 0) {
                permissions = yield permissionRepo.find({ name: { $in: permissions } });
                data.permissions = permissions.length > 0 ? permissions.map((p) => p.getId()) : [];
            }
            else {
                data.permissions = [];
            }
    ...
    schema_1.UserTC.addResolver({
        ...
        resolve: ({ obj, args, context, info }) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (items.length > 0) {
                const item = items[0];
                const data = lodash_1.merge(transform(items[0]), args.input);
                item.set(data);
    ...
    schema_1.UserTC.addResolver({
        ...
        resolve: ({ obj, args, context, info }) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (items.length > 0) {
                ...
                const data = lodash_1.merge(transform(items[0]), { password: args.newPassword });
    ...
    schema_1.UserTC.addResolver({
        ...
        resolve: ({ obj, args, context, info }) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (items.length > 0) {
                const item = items[0];
                const data = lodash_1.merge(transform(items[0]), { email: args.newEmail });
                item.set(data);
                if (yield repository.update([item])) {
                    return true;
                }
    ...
    schema_1.UserTC.addResolver({
        ...
        resolve: ({ obj, args, context, info }) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (permissions.length > 0) {
                permissions = yield permissionRepo.find({ name: { $in: permissions } });
                data.permissions = permissions.length > 0 ? permissions.map((p) => p.getId()) : [];
                permissionNames = permissions.length > 0 ? permissions.map((p) => p.data.name) : [];
            }
            else {
                data.permissions = [];
            }
