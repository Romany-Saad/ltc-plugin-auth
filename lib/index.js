    ...
    load(container) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            container.emitter.on(core_1.names.EV_PLUGINS_LOADED, (items) => __awaiter(this, void 0, void 0, function* () {
                // get all users
                const currentUsers = yield users.find({}, 0);
                // get all permissions
                const currentPermissions = yield permissions.find({}, 0);
                // for every user replace his permissions with array of objects
                let toBeUpdated = [];
                for (let user of currentUsers) {
                    if (typeof user.data.permissions[0] === 'string') {
                        const newPermissions = user.data.permissions.map((p) => {
                            //  get current permission
                            const perm = currentPermissions.find((permission) => p === permission.getId());
